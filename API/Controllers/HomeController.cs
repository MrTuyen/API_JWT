    ...
    public class HomeController : BaseApiController
    {
        ...
        public HttpResponseMessage CancelInvoice(HttpRequestMessage request)
        {
            try
            {
                ...
                DateTime.TryParse(objData.CANCELTIME.ToString("dd/MM/yyyy"), out DateTime calcelTime);

                var invoice = invoiceBLL.GetInvoiceByIdAPI(objData.ID);
                if (invoice == null)
                    return Request.CreateResponse(HttpStatusCode.OK, new { msg = $"Không tìm thấy hóa đơn {objData.ID}." });
                if (invoice.INVOICESTATUS != (int)DS.Common.Enums.EnumHelper.INVOICE_STATUS.CONFIRMED)
                    return Request.CreateResponse(HttpStatusCode.OK, new { msg = $"Hóa đơn chưa phát hành {objData.ID}." });
                if (calcelTime < invoice.SIGNEDTIME)
                    return Request.CreateResponse(HttpStatusCode.OK, new { msg = $"Thời gian hủy {objData.CANCELTIME} không nhỏ hơn thời gian phát hành {invoice.SIGNEDTIME.ToString("dd/MM/yyyy")} hóa đơn {objData.ID}." });
                ...
                return Request.CreateResponse(HttpStatusCode.OK, new { msg = message });
            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { rs = false, code = 104, msg = ex.ToString() });
        ...
        public HttpResponseMessage ViewPDF(HttpRequestMessage request)
        {
            ...
            try
            {
                ...
                return Request.CreateResponse(HttpStatusCode.OK, new { msg = $"Thành công.", data = new { rs = result["rs"], link = uriFolder + result["msg"] } });
            }
            catch (Exception ex)
            {
                return Request.CreateResponse(HttpStatusCode.OK, new { rs = false, code = 104, msg = ex.ToString() });
        ...
        public HttpResponseMessage CheckReleaseByInvoiceCode([FromBody] InvoiceBO inv)
        {
            try
            {
                InvoiceBLL invoiceBLL = new InvoiceBLL();
